...
def login_window(parent):
    ...
    login_frame.grid(row=0, column=0, padx=100, pady=50, sticky="WE")

    # Welcome Label
    welcome_text = tkboot.Label(login_frame, text="Expense Tracker", font=("", 20, "bold"), background="#20374C")
    ...
    exit_button = tkboot.Button(login_frame,
                            text="Exit",
                            style='danger.TButton',  # You can customize the style as needed
                            takefocus=False,
                            width=20,
                            command=signin_window.destroy)  # Close the sign-in window
    exit_button.grid(row=5, column=0, pady=10, ipady=5)

    # Set up layers
    signin_window.columnconfigure(0, weight=10)
    login_frame.columnconfigure(0, weight=10)